# Adopt oRPC for Generated API Service Boundaries: Teams Extend Orpc
...
- **R-ORPC-001** MAY: Teams MAY extend the oRPC framework with custom middleware or plugins for cross-cutting concerns like authentication or logging.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Generated contracts must be validated as up-to-date with source definitions before accepting API boundary changes. Manual edits to orpc.gen.ts files must be flagged and rejected.
</enforcement>