# Enforce External Client Boundary Isolation in Internal APIs: Configuration External Clients
...
- **R-EX-001** SHOULD: Configuration for external clients (credentials, endpoints, timeouts) SHOULD be externalized and not hardcoded in internal API implementations.

### Verify

```bash
# Verify no direct external client imports in internal service/core modules outside adapters
grep -r 'import boto3' api/services/ api/core/ --exclude-dir=tests | grep -v 'storage/' | wc -l | test $(cat) -eq 0

# Verify storage, telemetry, and plugin extensions define abstract interfaces
