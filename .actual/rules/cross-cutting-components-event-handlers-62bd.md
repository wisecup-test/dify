# Adopt 'use client' Directive for Client-Side Hydration in React Server Components: Components Event Handlers
...
- **R-RSC-001** MUST: Components with event handlers (onClick, onChange, onSubmit, etc.) MUST be marked with 'use client' directive as the very first line of the file, before any imports or other code.
...
- The 'use client' directive appears as the first line in all affected component files

<enforcement>
Claude Code MUST NOT skip or defer verification. All components with event handlers MUST be verified to include the 'use client' directive before acceptance.
</enforcement>